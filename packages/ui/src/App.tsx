...
export function AppShell() {
   ...
   const goTo = useCallback(
      (to: "/" | "/agent-setup" | "/skills") => {
         const tokenizedPath = token
            ? `${to}?t=${encodeURIComponent(token)}`
            : to;
   ...
   return (
      <RatelAppContext.Provider value={context}>
         <div className="min-h-dvh">
            ...
            />
            <div className="mx-auto flex max-w-7xl flex-col md:flex-row border-r border-border">
                  ...
                  {!token ? (
                     <main className="w-full px-4 py-6 sm:px-6">
                        <Alert>
                           ...
                           <AlertDescription>
                              Open the URL printed by ratel-mcp ui.
                           </AlertDescription>
                        </Alert>
                     </main>
                  ) : (
         ...
         <CommandMenu
            ...
            onAddToolSource={() => {
               ...
               void navigate({
                  to: toolSourceCreatePath("user", token),
               } as never);
            }}
            onImport={() => {
               setCommandOpen(false);
...
const NAV_ITEMS: Array<{
   ...
   },
   {
      ...
      icon: <Settings2 />,
      isActive: (pathname) =>
         pathname === "/agent-setup" || pathname.startsWith("/agent-setup/"),
   },
   {
      to: "/skills",
      label: "Skills",
...
function SessionMenu(props: { config: ConfigResponse | null }) {
   ...
   const projectLabel =
      compactPathLabel(props.config?.projectRoot) ?? "No project root";
            ...
            <Command>
               ...
               <CommandList>
                  ...
                  <CommandGroup heading="Navigate">
                     ...
                     <CommandItem
                        onSelect={() => props.onNavigate("/agent-setup")}
                     >
                        <Settings2 />
                        Agent Setup
...
function commandMcpItems(config: ConfigResponse | null) {
   return SCOPES.flatMap((scope) => {
      ...
      return Object.entries(scopeState.config.mcpServers).map(
         ([name, entry]) => ({
            entry,
            name,
            scope,
...
function commandAgentStatus(host: DetectedAgentHostSummary): {
   ...
   if (host.posture === "unavailable")
      return { label: "Unavailable", tone: "muted" };
   if (
      host.ratelEntryCount > 0 &&
      (host.missingRatelEntryNames?.length ?? 0) === 0
   ) {
...
function CommandStatusBadge(props: {
   children: ReactNode;
   tone: "muted" | "success" | "warning";
}) {
   return (
...
function isLogResult(value: unknown): value is { log: string[] } {
   return (
      typeof value === "object" &&
      value !== null &&
      Array.isArray((value as { log?: unknown }).log)
   );
}

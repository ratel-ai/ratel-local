...
describe("ClaudeCodeAgentHostAdapter", () => {
  ...
  it("rewrites user and local entries in one home config write", async () => {
    ...
    expect(after.mcpServers["ratel-local"]).toEqual({
      type: "stdio",
      command: "ratel-local",
    ...
    expect(after.projects[ROOT].mcpServers["ratel-local"].args).toEqual([
      "serve",
      "--config",
      RATEL_USER,
      "--config",
  ...
  it("rewrites project entries with the user and project config chain", async () => {
    ...
    expect(after.mcpServers.keep).toEqual({ type: "stdio", command: "keep" });
    expect(after.mcpServers["ratel-local"].args).toEqual([
      "serve",
      "--config",
      RATEL_USER,
      "--config",

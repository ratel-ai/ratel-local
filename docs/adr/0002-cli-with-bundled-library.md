...
The `@ratel-ai/mcp-server` package serves two distinct consumption modes:
...
- **CLI**: a user running `npx @ratel-ai/mcp-server serve` (or `mcp add` / `mcp import` / etc.) without writing any TypeScript — the package itself owns the entrypoint.
...
1. **Two packages** — `@ratel-ai/mcp-server` (library) and `@ratel-ai/mcp-server-cli` (CLI depending on the library). Clean separation of concerns; doubles the version-bump surface area and forces two `npm install`s for the standalone CLI use case.
2. **CLI as a separate scope** — `@ratel-ai/ratel-mcp` (CLI) + `@ratel-ai/mcp-server` (library). Same lockstep pain plus a confusing naming split.

## Decision

Ship the library and the CLI from a single npm package, `@ratel-ai/mcp-server`. The package manifest:

- `main` and `types` point at the library entrypoint (`./dist/index.js` / `./dist/index.d.ts`).
- `bin: { "ratel-mcp": "./dist/bin.js" }` exposes the CLI under the global command `ratel-mcp` (and via `npx @ratel-ai/mcp-server ...`).
...
- Library-only consumers pay no observable cost: tree-shakers drop the `src/cli/` subtree from their bundles since it's never reached from `src/index.ts`. Node runtime startup for `import { createMcpServer } from "@ratel-ai/mcp-server"` only loads what's reachable.

...
### Changed
- **Agent setup commands now live at the top level as `ratel-local import` and `ratel-local link`.** Import now spans both MCP entries and native skills, so neither command is nested under the MCP-only command group.
- **`ratel-local link` and `ratel-local import` now install the Claude Code statusline automatically** once they finish wiring up Claude Code, instead of requiring a separate `ratel-local statusline install` step. A pre-existing non-Ratel statusline is left untouched (reported as a note, not an error).

  ...
  if (agentState && plan.agentChanges.length > 0 && bin) {
    ...
    if (!opts.yes) {
      const ok = await ctx.prompts.confirm({
        ...
        } with the ratel-local entry?`,
        initialValue: true,
      });
      if (ctx.prompts.isCancel(ok) || ok === false) {
        ctx.log(
          `${agentState.host.displayName} config changes skipped. Run \`ratel-local link\` (or re-run \`ratel-local import\`) to point ${agentState.host.displayName} at Ratel later.`,
        );
        ctx.prompts.outro(
...
function renderSummary(
  ...
  if (plan.summary.overwrittenRatelEntries.length > 0) {
    lines.push("");
    lines.push(
      `Overwriting existing ${agentHostName} ratel-local entry at: ${plan.summary.overwrittenRatelEntries.join(
        ", ",
      )}`,
    );
  }
...
function renderAgentStage(plan: ImportPlan, hostName = "agent"): string {
  ...
  lines.push("");
  lines.push(
    `${hostName} MCP entries now managed by Ratel will be replaced by a single ratel-local entry. Other ${hostName} MCP entries are preserved.`,
  );
  if (plan.summary.skipped.length > 0) {
    lines.push("");
    lines.push("Not copied into Ratel:");
